...
class IndexAutoPurchaseMonthCron extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'index-auto-month:cron';
    ...
    public function handle()
    {
        ...
        foreach ($autoPurchases as $autoPurchase) {
            ...
            $reportCreatedAt = Carbon::parse($lastReport->created_at ?? Carbon::now()->subMonths(2));
            // Проверяем, прошло ли 7 дней с момента создания отчета
            if ($currentDateTime->diffInMonths($reportCreatedAt) >= 1) {
                ...
                $result = app('App\Http\Controllers\V1\IndexController')->buyVTIfromHandler($requestData);
